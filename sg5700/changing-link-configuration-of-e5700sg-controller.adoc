...
xref:placing-appliance-into-maintenance-mode.adoc[將E5700SG控制器置於維護模式]。
...
+
如果您變更了VLAN設定、裝置的子網路可能已經變更。如果您需要變更應用裝置的IP位址、請遵循 xref:setting-ip-configuration-sg5700.adoc[設定IP組態] 說明。

. 從《SectionAppliance安裝程式》StorageGRID 中、選取*「Configure Networking」（設定網路）*>「* Ping Test」（* Ping測試）*。
. 使用Ping測試工具來檢查任何網路上的IP位址連線能力、這些網路可能會受到您在中所做的連結組態變更的影響 ,變更連結組態 步驟。
+
