...
+
*** 若要解決憑證本身的問題、例如更新過期的憑證、請參閱 xref:../admin/kms-configuring.adoc[設定KMS的說明]。
*** 如果連線至KMS主機時發生非預期的問題、請確認 xref:checking-dns-server-configuration.adoc[網域名稱系統（DNS）伺服器正確] 以及這點 xref:configuring-network-links-sg100-and-sg1000.adoc[應用裝置網路設定正確]。
...
** 選取*重新開機進入維護模式*、以重新啟動控制器、使節點保持維護模式。（此選項僅在控制器處於維護模式時可用。） 如果您需要在節點上執行其他維護作業、才能重新加入網格、請選取此選項。
+
