...
NOTE: 此程序適用於具有使用命令列介面經驗的進階使用者。或者、您也可以 xref:automating-appliance-configuration-using-storagegrid-appliance-installer.adoc[使用StorageGRID 「功能不全」安裝程式來自動化組態]。
...
. 如果您要將應用裝置組態備份到Json檔案、請使用進階和「備份檔案」子命令。例如、如果您要將IP位址為「_SGA-install-IP_」的應用裝置組態備份到名為「appliance-SG1000.json」的檔案、請輸入以下命令：+「configure-sga.py進階-備份檔案應用裝置-SG1000.json _SGGA安裝-IP_」
+
