...
. 對連結組態進行所需的變更。
+
如需選項的詳細資訊、請參閱 xref:configuring-network-links-sg100-and-sg1000.adoc[設定網路連結]。
...
. 對應用裝置的IP位址進行必要的變更。
+
如果您變更了VLAN設定、裝置的子網路可能已經變更。如果您需要變更應用裝置的IP位址、請參閱 xref:configuring-storagegrid-ip-addresses-sg100-and-sg1000.adoc[設定StorageGRID 靜態IP位址]。
